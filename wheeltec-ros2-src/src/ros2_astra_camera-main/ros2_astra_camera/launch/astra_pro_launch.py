...
def generate_launch_description():
    ...
    parameters_path_depth = os.path.join(
        package_dir, 'launch', 'params', 'astra_pro_depth.yaml')
    parameters_path_uvc = os.path.join(
        package_dir, 'launch', 'params', 'astra_pro_uvc.yaml')
    ...
    usb_cam = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(camera_dir, 'launch','usb_cam_launch.py')),
    )
    start_astra_camera_node_cmd = Node(
        ...
        output="screen",
        emulate_tty=True,
        parameters=[parameters_path_depth]
    )
